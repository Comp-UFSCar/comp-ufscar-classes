...
boundedPathFinding = function(map, x, y, character, radius, canSwim) {
	...
	while(stack.length>0) {
		...
		// check to see if there is a bridge, otherwise, a character would not be able to get out of there:
...
function BRProtectPossessions() {
	...
	this.update = function(character, map, time, game) {
		if (this.state.map==null) {
			...
			var l = map.objectsInRadius(character.x,character.y,character.sightRadius);
