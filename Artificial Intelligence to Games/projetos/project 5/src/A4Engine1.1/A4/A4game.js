...
Game.updateGame = function() {
  ...
  if (this.currentCharacter>=0) {
    ...
    if (keyPressed(KEY_SHIFT) && character.state == STATE_READY) {
      ...
      if (direction!=null) {
        // check if there is a character in the desired position, and if there is check for potential dialogue actions
        var target = currentMap.getCharacter(target_x, target_y);
        if (target!=null && character.talk_state == STATE_READY && !(target instanceof Enemy)) {
      ...
      if (character.talk_state == STATE_READY && keyPress(KEY_TALK)) { // T: for talking
        ...
        var target = currentMap.findClosestCharacterToTalk(character);
        this.talkDialogue(character, target, currentMap);
      }
...
//                                  console.log("Drop coins dialogue finished with value '" + value + "'");
    ...
    if (keyPress(KEY_ZOOM)) {
      if (TARGET_TILE_WIDTH==32) {
        TARGET_TILE_WIDTH = 64;
        TARGET_TILE_HEIGHT = 64;
//      } else if (TARGET_TILE_WIDTH==64) {
//        TARGET_TILE_WIDTH = 16;
//        TARGET_TILE_HEIGHT = 16;
      } else {
        TARGET_TILE_WIDTH = 32;
        TARGET_TILE_HEIGHT = 32;
      }
    }
...
//            if (this.storyState[rule.variable] == rule.value) triggered = true;
  ...
  if(this.storyStateRuleActionQueue.length>0) {
    var retVal = executeRuleEffect(this.storyStateRuleActionQueue[0], null, null, this, null);
