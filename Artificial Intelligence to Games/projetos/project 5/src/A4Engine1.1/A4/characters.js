...
//			console.log("Loading animation for " + animationName);
		...
		for(var i = 0;i<attributesElements.length;i++) {
			...
			this[name] = value;
			if (name=="hp") this.max_hp = value;
			if (name=="mp") this.max_mp = value;
		}
						...
						for(i = 0;i<this.inventory.length;i++) {
							...
							if (item instanceof Key) {
								if (item.keyID == door.name) {
									...
									map.updateVisibility(this.x,this.y);
									return true;
								}
							}
						}
				...
				if (items.length>0) {
					...
					if (item != null) {
						...
						for(var i = 0;i<game.characters.length;i++) {
							map.updateVisibility(game.characters[i].x,game.characters[i].y);
						}
					} else {
...
//				console.log("ACTION_DROP_GOLD " + action.gold);
				if (this.gold<action.gold) {
					...
					var item = new ItemCoinPurse(action.gold, COINPURSE_TILE);
...
//				console.log("ACTION_UNEQUIP " + action.item);
				...
				if (action.spell == SPELL_MAGIC_MISSILE) {
					if (action.direction==DIRECTION_LEFT ||
						...
						game.warpObject(new Spell(action.direction, 6, Math.floor(this.getMagicMultiplier()*4), 0, this, 115), 
					...
					if (action.direction==DIRECTION_LEFT ||
					    ...
					    action.direction==DIRECTION_DOWN) {
						this.mp-=spell_cost[action.spell];
						game.warpObject(new Spell(action.direction, 10, Math.floor(this.getMagicMultiplier()*12), 1.5, this, 125), 
										this.x+action.offsx,this.y+action.offsy, this.map, 1);
						this.state = STATE_CASTING;
						this.state_timer = 0;
					}
					...
					if (action.direction==DIRECTION_LEFT ||
					    ...
					    action.direction==DIRECTION_DOWN) {
						this.mp-=spell_cost[action.spell];
						game.warpObject(new Spell(action.direction, 24, Math.floor(this.getMagicMultiplier()*32), 0, this, 135), 
										this.x+action.offsx,this.y+action.offsy, this.map, 1);
						this.state = STATE_CASTING;
						this.state_timer = 0;
					}
							...
							for(ty = bridge.yTarget;ty<bridge.yTarget+bridge.heightTarget;ty++) {
								if (targetMap.walkable(tx,ty,this)) {
							        game.warpObject(this, tx, ty, bridge.mapTarget, 2);
				                  	...
				                  	targetMap.updateVisibility(this.x,this.y);
				                  	return true;
								}
							}
						}
			...
			if (action.type == ACTION_TAKE) {
				...
				if ((this.vehicle.canSwim && map.sailable(nextx,nexty,this)) ||
					...
					if (bridge != null) {
						...
						if (this.player_character) game.pushMessage("Something is blocking the way...");
						this.state = STATE_READY_VEHICLE;
						this.x = oldx;
						this.y = oldy;
						return false;
					}
	...
	this.update = function(map, game) {
		if (this.hp<=0) {
			...
			if (this.gold>0) {
				item = new ItemCoinPurse(this.gold, COINPURSE_TILE);
				game.warpObject(item, this.x, this.y, this.map, 1);
			}
