...
/**
 * @author Santi: AI that builds a barracks and lumbermill, two peasants harvesting gold, and two harvesting wood, 
 *         then builds
 * 	       archers without stop and attacks! After a while, increases the number of peasants to 3 and 3, and builds a
 * 		   second barracks. It also looks for goldmines where there is still available gold
 * 		   Also, it sends archers in waves
 */
public class ArchersRush extends FootmenRush {


	public ArchersRush(String playerID) {
		super(playerID);
		troopClass = WArcher.class;
	}

	List<Request> checkBarracks(S3 game, WPlayer player, List<S3Action> actions) {
		Class []buildOrder = {WLumberMill.class,WBarracks.class};
		List<Request> requests = new LinkedList<Request>();
		if (DEBUG >= 1) System.out.println("ArchersRush: checkBarracks");
